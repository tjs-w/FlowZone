...
- A dedicated `ui://flowzone/dyna/v19.html` resource, with v18 retained as a compatibility alias, below a 960 KiB payload budget with a closed network CSP and clipboard-write permission for explicit copy actions. Oxanium identifies sparse dashboard landmarks, Geist carries readable working text and controls, and Geist Mono is reserved for comparable machine-state data. Ordinary external-link clicks use one host navigation request, while the real anchor preserves modified clicks, right-click, copy-link, and text selection. Its bounded context menu prioritizes exact selected text, links, notes, Codex tasks, work items, and dashboard actions; unselected editable fields retain the host's native editing menu.
