...
`ui://flowzone/v5.html` remains the Markdown Review output resource. `ui://flowzone/dyna/v6.html` is a separate, smaller Dyna resource with no inherited clipboard permission. Public model output stays small; private UI data uses a typed metadata envelope:
