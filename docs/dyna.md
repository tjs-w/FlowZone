...
- Each new scheduled publisher registers the complete immutable set of source slices it must report, so a run cannot silently omit a planned connector; legacy publishers without a manifest remain compatible.
...
The reviewed Apps SDK UI package does not currently provide dense ledger rows, a pipeline rail, a data grid, or a responsive side-inspector primitive. The component choice was re-measured against the current 750 KiB single-resource budget: the complete Dyna HTML, JavaScript, and CSS resource is 750,725 bytes, while adding the json-render browser renderer costs about 49 KiB, `SegmentedControl` about 31 KiB, `Menu` about 106 KiB, and `Select` about 116 KiB. `EmptyMessage` fits at about 2 KiB but provides no semantic or density improvement over Dyna's action-aware empty state. The incremental component measurements are bundled JavaScript and CSS, not npm package sizes.
