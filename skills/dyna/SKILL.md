...
3. Use `credentialMode: "disabled"` (the default) for a production-intended setup. Dyna discards the one-time local credential and returns no secret. Create the native scheduled task **paused** with a credential-free prompt, call `bind-schedule` with `scheduleState: "paused"` and the same `requiredSourceSlices`, and tell the user that it cannot be resumed until protected host authentication exists. Do not call `publish-run` for this setup.
...
5. The production path is a Streamable HTTP MCP endpoint authenticated through host-managed OAuth, so the scheduled task never receives a raw publisher secret. Local stdio publishing remains host-blocked because the current schedule host does not provide FlowZone with a protected schedule identity or per-schedule credential. Do not imply that local stdio is production-ready or emulate identity in the prompt.
6. Call `bind-schedule` once per dashboard/publisher pair with the exact native schedule ID, title, permitted state, the immutable required source manifest, and a `staleAfterMinutes` SLA appropriate to its recurrence (the default is 1,440 minutes). A publisher can feed multiple dashboards, and a dashboard can receive multiple publishers. Use `update-schedule-status` after pausing, resuming, renaming, or changing the SLA; it may register a manifest only when none exists and otherwise must repeat the exact manifest. Use `unbind-schedule` to retire one dashboard binding and `list-publishers` to reconcile inventory.
7. An authorized preview job gathers source data with the permitted email, messaging, source-control, TWG, skill, or Codex capabilities, then calls `publish-run` with no more than 200 bounded records.
