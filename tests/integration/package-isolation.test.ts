...
describe("isolated shipping package", () => {
  test("reconciles an exact installed Dyna rule for the shared default store", async () => {
    ...
    expect(rule).toContain('["update", "enrich", "complete"]');
    expect(rule).toContain('["add", "edit", "delete"]');
    expect(rule).toContain('        "place",');
    expect(rule).not.toContain('"place-many"');
    expect(rule).toContain('["archive", "restore"]');
    expect(rule).not.toContain('"todo"');
