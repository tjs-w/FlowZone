...
- Replaced Dyna's accidental host DevTools right-click with a compact, accessible, context-specific action menu for selected text, links, linked Codex tasks, work items, and dashboard background; preserved native editing and touch selection menus, added keyboard invocation and developer-only bypass semantics, and added an explicit authoritative refresh action. Advanced the Dyna resource cache key to `ui://flowzone/dyna/v12.html` while retaining the prior two aliases.
- Added direct, accessible workflow controls to Dyna: every active item's status opens a native menu, taskless items can move between To Do and Needs You or complete with a required outcome, and desktop users can drag Progress cards across lanes through the same guarded transition path. Linked items continue to open, start, or verify Codex work instead of manufacturing controller state. Advanced the SQLite schema to v7 and the Dyna resource cache key to `ui://flowzone/dyna/v11.html` while retaining the prior resource alias.
- Added a lazy side-panel Codex session picker that lists only bounded app-private task metadata, lets the user select an exact task and host, re-verifies that native task through the existing leased action bridge, and associates it without persisting unrelated task inventory or transcripts.
- Added the bundled `dyna` item CLI and `$flowzone:dyna` task-update workflow for safe cross-session synchronization: copied prompts carry versioned non-secret item references, work updates and typed artifacts remain append-only, exact-version mutations are retry-safe, and task-authored completion stays pending until controller-verified success. A one-time reconciler now installs narrowly scoped user-layer rules for the exact cached launcher and removes the obsolete path after plugin updates. Advanced the SQLite schema to v6, snapshot to `dyna/snapshot-v5`, UI payload to `dyna/ui-v7`, and resource cache key to `ui://flowzone/dyna/v10.html`.
