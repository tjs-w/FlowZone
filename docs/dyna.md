...
The dedicated `render_dyna_dashboard` tool renders `ui://flowzone/dyna/v13.html`. It is separate from Markdown Review so Dyna does not inherit Mermaid's bundle weight. It keeps a closed network CSP and requests clipboard-write permission only for explicit copy actions. A bounded, static context menu gives exact selected text, links, linked Codex tasks, work items, and the dashboard context-appropriate actions while unselected editable fields retain the host's native editing menu; touch-generated selection menus also stay native. Its combined checked-in HTML, JavaScript, and CSS budget is 870 KiB, including locally embedded Latin variable fonts so Remote/mobile rendering never depends on a font CDN. The versioned URI is the host cache key and must change whenever the shipped UI bundle changes materially.
