...
render_dyna_dashboard ───────────────> ui://flowzone/dyna/v17.html
...
`ui://flowzone/v5.html` remains the Markdown Review output resource. `ui://flowzone/dyna/v17.html` is a separate, smaller Dyna resource with a closed network CSP and clipboard-write permission limited to explicit copy actions; v16 remains a compatibility alias. Public model output stays small; private UI data uses a typed metadata envelope. Treat each UI resource URI as a host cache key and bump its version whenever the shipped HTML, JavaScript, or CSS changes materially.
